...
public class GalleryActivity extends AppCompatActivity {
  ...
  private void initView(Bundle savedInstanceState) {
    ...
    RxDebounceClick.onClick(previewBtn).filter(new Func1<Void, Boolean>() {
  ...
  private void navigateToPreview(List<ImageEntity> imageEntities) {
    ...
    Bitmap scaledBitmap =
        Bitmap.createScaledBitmap(screenSnapshot, screenSnapshot.getWidth() / BLUR_SCALE,
  ...
  @CheckResult private Bitmap catchScreen() {
    ...
    View rootView =
        GalleryActivity.this.getWindow().getDecorView().findViewById(android.R.id.content);

    rootView.setDrawingCacheEnabled(true);
    Bitmap drawingCache = rootView.getDrawingCache();

    if (screenBitmap == null || canvas == null || paint == null) {
      screenBitmap = Bitmap.createBitmap(drawingCache.getWidth(), drawingCache.getHeight(),
